...
const DEFAULT_DATA: StayHealthyTargetData = {
  primaryTarget: "Eat healthier",
  measurableTarget: "At least 15 min 3 x a week",
...
const MEASURABLE_OPTIONS = [
  "At least 15 min 3 x a week",
  "At least 30 min 3 x a week",
  "At least 45 min 3 x a week",
  "At least 60 min 3 x a week",
];
    ...
    <div className="relative mx-auto flex w-full max-w-[1080px] flex-col items-center">
      ...
      <div className="mt-10 flex justify-center">
        <div className="relative h-[120px] w-[120px] overflow-hidden rounded-full border border-[#eceff7] bg-white shadow-[0_8px_24px_rgba(24,33,77,0.06)]">
          <Image
            src="/images/goal-stay-healthy.png"
...
export default function StayHealthyWorkflow({
  ...
  const [formData, setFormData] = useState<StayHealthyTargetData>(DEFAULT_DATA);
  ...
  const toggleDay = (day: string) => {
    setFormData((prev) => {
      const exists = prev.frequencyDays.includes(day);
      const updatedDays = exists
        ? prev.frequencyDays.filter((d) => d !== day)
        : [...prev.frequencyDays, day];
      return { ...prev, frequencyDays: updatedDays };
    });
  };
  ...
  const validateStep = (currentStep: number) => {
    switch (currentStep) {
      case 1:
        return (
          formData.primaryTarget.trim().length > 0 &&
          formData.measurableTarget.trim().length > 0
        );
      case 2:
        return formData.frequencyDays.length > 0;
      case 3:
  ...
  const handleNext = () => {
    if (!validateStep(step)) {
      if (step === 2 && formData.frequencyDays.length === 0) {
        setErrorMessage("Please select at least one day for your frequency schedule.");
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const startDate = resolveStartDate(formData.startDate);
      const endDate = new Date(startDate);
      endDate.setDate(endDate.getDate() + 84);

      const description = [
        `Target: ${formData.primaryTarget}.`,
        `Measurable Target: ${formData.measurableTarget}.`,
        `Frequency: ${formData.frequencyDays.join(", ")}.`,
      ...
      const metadata = {
        ...
        measurable_target: formData.measurableTarget,
  ...
  return (
    <>
      <div className="w-full">
        ...
        {/* Step 1: Define Your Health Target */}
        {step === 1 && (
          <StepShell
            currentStep={1}
            title="Define Your Health target"
            goalTitle={goalTitle}
            icon={
              <svg
                width="22"
          ...
          >
            <div className="flex flex-col gap-6">
              <SelectQuestion
                ...
                value={formData.primaryTarget}
                onChange={(val) =>
                  setFormData((prev) => ({ ...prev, primaryTarget: val }))
                }
                options={TARGET_OPTIONS}
              />
              <SelectQuestion
                label="What's your measurable target?"
                value={formData.measurableTarget}
                onChange={(val) =>
                  setFormData((prev) => ({ ...prev, measurableTarget: val }))
                }
        ...
        {step === 2 && (
          ...
          >
            <div className="flex flex-col gap-6">
              <div>
                ...
                <div className="mt-3 flex flex-col gap-2.5">
                  {DAYS_OF_WEEK.map((day) => {
                    const isChecked = formData.frequencyDays.includes(day);
                    return (
                      <label
                        key={day}
                      ...
                      >
                        <input
                          ...
                          className="h-5 w-5 rounded border-gray-300 text-[#7655fb] focus:ring-[#7655fb]"
                        />
                        <span className="text-[16px] text-[#262525] font-secondary">
                          {day}
                        </span>
                      </label>
                    );
              ...
              <SelectQuestion
                label="Commitment Start Date & Time (Optional)"
